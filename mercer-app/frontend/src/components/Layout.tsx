...
// Canonical nav across all ODI demos. Persona-first links (industry-
// specific) + two dropdown clusters at the end:
//   • dbt-Wizard ▾ — narrative (Overview / Scenario / Live / Outcome)
//   • ODI ▾       — plumbing  (Architecture / Pipeline / About / OT-IT Policy)
type NavEntry =
  | { kind: 'link'; to: string; label: string }
  | { kind: 'group'; label: string; rootTo: string; matchPrefixes: string[]; children: { to: string; label: string }[] };

const NAV: NavEntry[] = [
  { kind: 'link', to: '/',             label: 'Plant Floor' },
  { kind: 'link', to: '/quality',      label: 'Quality' },
  { kind: 'link', to: '/predictive',   label: 'Predictive Maintenance' },
  { kind: 'link', to: '/sustainability', label: 'Sustainability' },
  { kind: 'link', to: '/related',      label: 'Related Parts' },
  {
    kind: 'group',
    label: 'dbt-Wizard',
    rootTo: '/dbt-wizard',
...
// Flat list for the mobile grid (groups become inline rows).
...
// ─── NavEntryEl — dark-theme variant (safety-yellow accent on graphite) ─────
  ...
  if (entry.kind === 'link') {
    return (
      <NavLink
        ...
        className={({ isActive }) =>
          `relative px-2.5 py-2 font-semibold tracking-tight text-[13px] whitespace-nowrap transition-colors ${
  ...
  return (
    <span ref={ref} className="relative inline-block">
      <button
        ...
        aria-haspopup="menu"
        aria-expanded={open}
        className={`relative px-2.5 py-2 font-semibold tracking-tight text-[13px] whitespace-nowrap inline-flex items-center gap-1 transition-colors ${
          isActive ? 'text-safety' : 'text-white/85 hover:text-white'
        }`}
      >
        {entry.label}
        ...
        {isActive && (
          <span className="absolute left-2.5 right-5 -bottom-[1px] h-[3px] bg-safety" />
        )}
      </button>
      {open && (
        <span role="menu" className="absolute left-0 top-full mt-1 min-w-[200px] border border-white/15 bg-graphite-900 shadow-xl overflow-hidden z-50">
            ...
            <NavLink
              ...
              className={({ isActive: ia }) =>
                `block px-4 py-2.5 text-[13px] font-medium transition-colors ${
                  ia ? 'bg-white/10 text-safety' : 'text-white/85 hover:bg-white/10 hover:text-white'
                }`
              }
            >
              {c.label}
